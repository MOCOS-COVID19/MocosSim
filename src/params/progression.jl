...
function make_severity_dists_ages(hosp_men_probs::Vector{T}, hosp_women_probs::Vector{T}, critic_probs::Vector{T}, hospitalization_multiplier::Float64 = 0.33, death_multiplier::Float64=1.8) where T<:Real
  n = length(hospitalization_women_probs)
  hosp_women_probs = hospitalization_multiplier* hosp_women_probs
  hosp_men_probs = death_multiplier*hospitalization_multiplier*hosp_men_probs
  critic_probs = death_multiplier*critical_probs
  ...
  for idx in 1:n
    ...
    severity_dists_ages[1][idx] = Categorical([0,  1-hosp_men_probs[idx],  hosp_men_probs[idx]*(1-critic_probs[group_ids]),  hosp_men_probs[idx]*critic_probs[group_ids]])
    severity_dists_ages[2][idx] = Categorical([0,  1-hosp_women_probs[idx],  hosp_women_probs[idx]*(1-critic_probs[group_ids]),  hosp_women_probs[idx]*critic_probs[group_ids]])
  end
  severity_dists_ages
end