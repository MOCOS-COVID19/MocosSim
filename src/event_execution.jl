...
function execute!(::Val{TransmissionEvent}, state::SimState, params::SimParams, event::Event)::Bool
  if Healthy != subjecthealth(state, event) || isimmune(state, params, subject(event), immunityof(state, subject(event)), strainkind(event)) # apparently the subject became already infected in some other way
    return false
  end
